...
exports.algoliaListingIndex = functions.firestore
    .document('listing/{listingId}')
    .onWrite((change, context) => {

        let index = algolia.initIndex('listing');
        //
        let listingId = change.after.id;
        let data = change.after.data();

        if (!data) {
            return index.deleteObject(listingId, (err) => {
                if (err) throw err;
                console.log('Listing Removed in Algolia Index', listingId)
            })
        }
